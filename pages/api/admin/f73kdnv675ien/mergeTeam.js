       ...
       console.log(usersWithNullTeamId.length);
        ...
        for (const team of totalTeams1) {
            if (team.members.length === 1) {
                const twoMemberTeam = totalTeams1.find(t => t.members.length === 2);
        ...
        for(const team of totalTeams1){
            if(team.members.length===1){
                const oneMemberTeam1=totalTeams1.find(t=>t.members.length===1 && t.teamName!=team.teamName);
        ...
        for (let i = 0; i < userobj.length; i += 3) {
            ...
            if (remainingUsers >= 3) {
                ...
                const existingThreeMemberTeams = await TeamModel1.find({ members: { $size: 3 } });
