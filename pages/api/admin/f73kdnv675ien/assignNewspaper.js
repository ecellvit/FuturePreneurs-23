       ...
       console.log(distribution)
        ...
        for (const teamName of shuffledTeams) {
            if (part1Distribution >= 0) {
                const team=await TeamModel1.findOne({teamName:shuffledTeams[part1Distribution]})
                //console.log(team);
               // const team = totalTeams.find(team => team.teamName === teamName);
                
                await TeamModel1.findOneAndUpdate({teamName:teamName}, {
                    $set: { newspaperLink: newspaperLinks[0],newspaperset:1,newspaperExists:true },
                });
                part1Distribution--;
            }
            ...
            if (part1Distribution === -1 && part2Distribution >= 0) {
                const team=await TeamModel1.findOne({teamName:shuffledTeams[part1Distribution]})
                await TeamModel1.findOneAndUpdate({teamName:teamName}, {
                    $set: { newspaperLink: newspaperLinks[1] ,newspaperset:2,newspaperExists:true},

                })
                part2Distribution--;

                if (part1Distribution === -1 && part2Distribution == -1 && part3Distribution >= 0) {
                    const team=await TeamModel1.findOne({teamName:shuffledTeams[part1Distribution]})
                    await TeamModel1.findOneAndUpdate({teamName:teamName}, {
                        $set: { newspaperLink: newspaperLinks[2],newspaperset:3,newspaperExists:true },
         ...
         const teamsWithoutNewspaper=await TeamModel1.find({newspaperExists:false});
         
         const teamNames1 = teamsWithoutNewspaper.map(team => team.teamName);
