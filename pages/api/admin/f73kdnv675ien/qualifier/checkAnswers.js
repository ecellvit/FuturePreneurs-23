  ...
  try {
    ...
    for (let pointer = 0; pointer < answers.easyAnswers.length; pointer++) {
      if (typeof answers.easyAnswers[pointer] === "number") {
        ...
        if (compareArrays(easyAnswers[pointer], answers.easyAnswers[pointer])) {
          points += gamePoints.easyPoints;
        }
      }
    }
    // for comparing medium answers
    for(let pointer=0; pointer< answers.mediumAnswers.length; pointer++){
        if(typeof answers.mediumAnswers[pointer] === "number"){
            if(mediumAnswers[pointer] === answers.mediumAnswers[pointer]){
                points += gamePoints.mediumPoints;
            }
        }
        else if(typeof answers.mediumAnswers[pointer] === "object"){
            if(compareArrays(mediumAnswers[pointer], answers.mediumAnswers[pointer])){
                points += gamePoints.mediumPoints;
            }
        }
    }
    // for comparing hard answers
    for(let pointer=0; pointer< answers.hardAnswers.length; pointer++){
        if(typeof answers.hardAnswers[pointer] === "number"){
            ...
            if(compareArrays(hardAnswers[pointer], answers.hardAnswers[pointer])){
                points += gamePoints.hardPoints;
            }
        }
    }
    // for comparing caseStudy answers
    for(let pointer=0; pointer< answers.caseStudyAnswers.length; pointer++){
        if(typeof answers.caseStudyAnswers[pointer] === "number"){
            if(caseStudyAnswers[pointer] === answers.caseStudyAnswers[pointer]){
                points += gamePoints.caseStudyPoints;
            }
        }
        else if(typeof answers.caseStudyAnswers[pointer] === "object"){
            if(compareArrays(caseStudyAnswers[pointer], answers.caseStudyAnswers[pointer])){
                points += gamePoints.caseStudyPoints;
            }
        }
    }
    ...
    return res.status(200).json({ message: "Points updated successfully", points: points });
  } catch (err) {
    console.log(err);
